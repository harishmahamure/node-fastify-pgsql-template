...
export async function authenticateJWT(req: FastifyRequest, res: FastifyReply) {
  try {
    const token = req.headers['x-authorization'] || req.headers['authorization']?.split(' ')[1]; // Support Bearer Token
    ...
    return res.code(401).send({ success: false, message: 'Authentication failed' });
  }
}
